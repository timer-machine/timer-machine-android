...
internal class VoiceDialog(private val context: Context) {
    ...
    fun requestVoiceContent(
        ...
        binding.text.text = buildString {
            append(context.getString(R.string.voice_content_info_empty))
            append("\n\n")
            append(context.getString(R.string.voice_content_info_variables_intro))
        }
        ...
        popupMenu {
                ...
                customItem {
                    ...
                    viewBoundCallback = { view ->
                        ...
                        view.findViewById<TextView>(android.R.id.content)
                            .setText(R.string.voice_content_group_variables)
                    }
                }
            }

            val groupVariable = context.getString(R.string.voice_group_variable)
            ...
            section {
                addItem(
                    ...
                    desp = context.getString(R.string.voice_loop),
                    isGroupVariable = true
                )
                addItem(
                    replacer = VoiceAction.REPLACER_TOTAL_LOOP,
                    desp = context.getString(R.string.voice_total_loop),
                    isGroupVariable = true
                )
                addItem(
                    replacer = VoiceAction.REPLACER_STEP_NAME,
                    desp = context.getString(R.string.voice_step_name)
                )
                addItem(
                    replacer = VoiceAction.REPLACER_STEP_DURATION,
                    desp = context.getString(R.string.voice_step_duration)
                )
            }
            section {
                title = context.getString(R.string.time_panel_elapsed_time)
                addItem(
                    replacer = VoiceAction.REPLACER_ELAPSED_TIME,
                    desp = context.getString(R.string.time_panel_elapsed_time)
                )
                addItem(
                    replacer = VoiceAction.REPLACER_ELAPSED_TIME_PERCENT,
                    desp = context.getString(R.string.time_panel_elapsed_percent)
                )
                addItem(
            ...
            section {
                title = context.getString(R.string.time_panel_remaining_time)

                addItem(
                    replacer = VoiceAction.REPLACER_REMAINING_TIME,
                    desp = context.getString(R.string.time_panel_remaining_time)
                )
                addItem(
                    replacer = VoiceAction.REPLACER_REMAINING_TIME_PERCENT,
                    desp = context.getString(R.string.time_panel_remaining_percent)
                )
                addItem(
            ...
            section {
                title = context.getString(R.string.voice_time_variable)
                addItem(
                    replacer = VoiceAction.REPLACER_CURRENT_TIME,
                    desp = context.getString(
                        R.string.voice_current_time,
                        TimeUtils.formattedTodayTime(context, 12, 34)
                    )
                )
                addItem(
                    replacer = VoiceAction.REPLACER_STEP_END_TIME,
                    desp = context.getString(R.string.time_panel_step_end_time)
                )
                addItem(
                    replacer = VoiceAction.REPLACER_TIMER_END_TIME,
                    desp = context.getString(R.string.time_panel_timer_end_time)
                )
                addItem(
                    replacer = VoiceAction.REPLACER_GROUP_END_TIME,
                    desp = context.getString(R.string.voice_group_end_time),
                    isGroupVariable = true
                )
            }
        }
